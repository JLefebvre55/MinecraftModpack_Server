...
function server_structure {
    ...
    echo 'Fetching modpack info...'
    download_file "${modpack_url}" true

    echo 'Creating server launch scripts...'
    echo "java -Xmx4G -jar server.jar nogui" > launch.sh
    echo "@echo off" > launch.bat
    echo "java -Xmx4G -jar server.jar nogui" >> launch.bat
}
...
function install_forge {
    ...
    # pushd "bin" > /dev/null
    forge_version=$(jq -r '.forgeVersion' "${workspace_path}/${build_folder}/${modpack_config}")
...
function install_minecraft {
    minecraft_version=$(jq -r '.minecraftVersion' "${workspace_path}/${build_folder}/${modpack_config}")
...
function read_mods {
    ...
    save_ifs=$IFS
    IFS=$'\n'
    url_download_list=($(jq -r '.mods.urls[] // {} | .url' "${workspace_path}/${build_folder}/${modpack_config}"))
    curse_mod_projectIds=($(jq -r '.mods.curse[] // {} | .projectId' "${workspace_path}/${build_folder}/${modpack_config}"))
    curse_mod_fileIds=($(jq -r '.mods.curse[] // {} | .fileId' "${workspace_path}/${build_folder}/${modpack_config}"))
    IFS=$save_ifs
}
...
echo "Created $server_output_zip"